...
function add(){
	window.location.href="adminVersionAdd";
}
...
function changeStatus(a){
	if(confirm("确定删除所选记录吗？")){
		$("#ids").val(a);
		$("#queryListForm").attr("action","${ctx}/manager/adminVersionDel");
		$("#queryListForm").submit();
	}else{
		return false;
	}
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#e2e2e2" >
        <tr >
          ...
          <td width="15%" height="40" bgcolor="e5e6e6" class="STYLE6"><div align="center"><span class="STYLE10">版本号</span></div></td>
          <td width="6%" height="40" bgcolor="e5e6e6" class="STYLE6"><div align="center"><span class="STYLE10">文件名称</span></div></td>
        ...
        <tr>
          ...
          <td height="40" bgcolor="#FFFFFF" class="STYLE6"><div align="center"><span class="STYLE19">${item.versionCode }</span></div></td>
          <td height="40" bgcolor="#FFFFFF" class="STYLE19"><div align="center">${item.downloadPath }</div></td>

          
            <td height="40" bgcolor="#FFFFFF" class="STYLE21"><div align="center"><a  class="jsxg"style="color:#00ce9b;" onclick="changeStatus('${item.id}')">删除</a> <a onclick="javascript:window.open('${ctx}/manager/adminVersionAdd?id=${item.id}','rightFrame')"  class="jsxg"style="color:#00ce9b;">修改</a> </div></td>
        </tr>
</c:forEach>
      </table></td>
  </tr>

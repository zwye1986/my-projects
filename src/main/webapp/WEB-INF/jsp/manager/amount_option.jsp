...
function add(){
	window.location.href="${ctx }/manager/amount/option/save";
}
...
function changeStatus(a){
	if(confirm("确定删除所选记录吗？")){
		$("#ids").val(a);
		$("#queryListForm").attr("action","${ctx }/manager/amount/option/delete");
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#e2e2e2" >
        <tr >
          ...
          <td width="15%" height="40" bgcolor="e5e6e6" class="STYLE6"><div align="center"><span class="STYLE10">金额</span></div></td>

          <td width="20%" height="40" bgcolor="e5e6e6" class="STYLE6"><div align="center"><span class="STYLE10">操作</span></div></td>
        </tr>
        
         <c:forEach items="${options }" var="item" varStatus="s">
        <tr>
          <td height="40" bgcolor="#FFFFFF"><div align="center">
             ${s.index+1 }
            </div></td>
      
          <td height="40" bgcolor="#FFFFFF"><div align="center" class="STYLE19" >${item.optionValue }</div></td>
          
           
            <td height="40" bgcolor="#FFFFFF" class="STYLE21"><div align="center"><a  class="jsxg"style="color:#00ce9b;" onclick="changeStatus('${item.id}')">删除</a> <a onclick="javascript:window.open('${ctx }/manager/amount/option/save?ids=${item.id}','rightFrame')"  class="jsxg"style="color:#00ce9b;">修改</a> </div></td>
        </tr>
</c:forEach>
      </table></td>
  </tr>

...
function changeStatus(a){
	if(confirm("确定删除所选记录吗？")){
		$("#ids").val(a);
		$("#queryListForm").attr("action","${ctx}/manager/adminRecommendGameDel");
		$("#queryListForm").submit();
	}else{
		return false;
	}
...
function EnterPress(gameId){
	var theEvent = window.event || arguments.callee.caller.arguments[0];
	 if (theEvent.keyCode==13) {
		 var orderBy = $("#"+gameId).val();
		 //验证只能是数字
		 var strP=/^\d+(\.\d+)?$/; 
		    if( !strP.test(orderBy) ){
		    	alert("请输入有效数字");
			    return false;
		    }
		    
		 var params = { id:gameId,orderBy:orderBy};
		   $.ajax({
		          type: "POST",
		          async: true,
		          data:params,
		          cache: false,
		          url: "${ctx}/manager/updateOrderBy",
        ...
        <td height="70" bgcolor="#f1f1f1" style="border-bottom:1px solid  #cbcbcb;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td><div align="right" style="height:54px; line-height:54px;"><span class="STYLE1">
             <a id="add" style="float:left; margin-left:15px;cursor:pointer;">   &nbsp; <a style="float:left;margin-left:15px;cursor:pointer;"> <img id="recommend" src="${ctx}/images/manager/images/xg.png" width="54" height="54" /></a><em id="recommend2" style="float:left;cursor:pointer;">游戏推荐 </em> &nbsp;&nbsp;  <a id="del" style="float:left;margin-left:15px;cursor:pointer;">  &nbsp;</span></div></td>
          </tr>
        </table></td>
      </tr>
    </table></td>
  </tr>
 <form id="queryListForm" name="queryListForm" target="_self" action="${ctx }/manager/adminRecommendGameList" method="post">
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#a8c7ce" >
      <tr>
        ...
        <td width="14%" height="40" bgcolor="e2dfd8" class="STYLE6"><div align="center"><span class="STYLE10">排序</span></div></td>
      </tr>
      <c:forEach items="${list }" var="item">
       <tr>
        ...
        <td height="20" bgcolor="#FFFFFF"><div align="center"   class="STYLE21"><a style="cursor: pointer;" onclick="changeStatus('${item.gameId}')">删除</a> | <a style="cursor: pointer;" href="${ctx}/manager/adminGameAdd?id=${item.id}"> 查看</a> | <a style="cursor: pointer;" href="${ctx}/manager/adminGamePolicyAdd?gameid=${item.id}"> 游戏策略</a></div></td>
        <td height="20" bgcolor="#FFFFFF" class="STYLE19"><div align="center"   class="STYLE21"><input type="text" value="${item.recommendOrderBy }" id="${item.gameId}" onkeydown="EnterPress('${item.gameId}')"></div></td>   
      </tr>
      </c:forEach>

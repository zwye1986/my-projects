...
.news_c{ width:100%;}
.news_c form{ width:800px; height:300px; }
...
$(document) .ready( 
		function() {
			...
			$("#successRate").blur(function() {
				  var re=/^\d+(?=\.{0,1}\d+$$)/;
				  var amount=$("#successRate").val();
				  if(amount!=""){
					  if(!re.test(amount)){
						  alert("数据输入有误！");
						  $("#successRate").val("");
...
function dealAdd() {
   if($("#advertiseId").val().length > 0){
	   $("#uuid").remove();
	   $("#addProjectInvestForm").attr("action",  "${pageContext.request.contextPath}/manager/projectInvestRate/update");
   }else{
	   $("#advertiseId").remove();
	   $("#addProjectInvestForm").attr("action",  "${pageContext.request.contextPath}/manager/projectInvestRate/save");
   }
	window.parent.location.href = "${ctx}/manager/projectInvestRateLists";
...
<table  align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
  <tbody>
        <tr>
          <td height="300">
          <div class="news_c" align="center">
         <form name="addProjectInvest" id="addProjectInvestForm" method="post">
			<input type="hidden" id="advertiseId" name="id" value="${projectInvestRate.id }">
   	      <div class="kv_item"><label class="news_add_list">利率设置名称:</label><input class="news_txt" type="text" name="rate_name" id=rate_name  value="${projectInvestRate.rate_name }"/></div>
          <div class="kv_item"><label class="news_add_list">时间间隔:</label><input class="news_txt" type="text" name="interval" id="interval" value="${projectInvestRate.interval }" /></div>
          <div class="kv_item"><label class="news_add_list">利率:</label><input class="news_txt" type="text" name="successRate" id="successRate"  value="${projectInvestRate.successRate }"/></div>
          <div class="kv_item"><label class="news_add_list">描述:</label></div>
          <div><textarea class="txt_area"  id="content" name="content" >${projectInvestRate.content }</textarea></div>

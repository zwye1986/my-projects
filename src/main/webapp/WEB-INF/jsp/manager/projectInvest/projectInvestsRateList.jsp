...
$(document).ready(function(){
	 ...
	 $("#add_resc").click(function(){
		 ...
		 $.showPanel("新增项目支持",
					"${ctx }/manager/projectInvestRate/view?projectId="+projectId,
					800, 340, true, "${ctx}");
	 });
...
function paginate(forward){
	...
	document.mainForm.submit();
		...
		function modifyDetail(id) {
			 $.showPanel("新增项目支持",
						"${ctx }/manager/projectInvestRate/view?ids="+id,
						800, 340, true, "${ctx}");
			return;
	}
		...
		function deletesingle(resId){
			if(confirm("您确定要删除选中的纪录吗？")){
				$.ajax({
					   ...
					   dataType: "json",
					   data:{ids:resId},
					   url: "${ctx}/manager/projectInvestRate/delete"
					});
				 window.location.href="${ctx}/manager/projectInvestRateLists";
			}
		}
...
<form action="${ctx }/manager/projectInvestRateLists" method="post" target="_self"
  ...
  <tr>
    <td><table id="data" width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#e2e2e2" >
        <tr ><input type="hidden" name="pageSize" id="page.pageSize"
          ...
          <td width="6%" height="40" bgcolor="e5e6e6" class="STYLE6"><div align="center"><span class="STYLE10">标识</span></div></td>
          <td width="13%" height="40" bgcolor="e5e6e6" class="STYLE6"><div align="center"><span class="STYLE10">时间间隔</span></div></td>
          <td width="6%" height="40" bgcolor="e5e6e6" class="STYLE6"><div align="center"><span class="STYLE10">利率</span></div></td>
          <td width="20%" height="40" bgcolor="e5e6e6" class="STYLE6"><div align="center"><span class="STYLE10">描述</span></div></td>
          <td width="20%" height="40" bgcolor="e5e6e6" class="STYLE6"><div align="center"><span class="STYLE10">操作</span></div></td>
          
         <tbody>
        <c:forEach items="${projectInvestRates}" var="projectInvestRate" varStatus="s" begin="0">
        <tr>
        <td height="40" bgcolor="#FFFFFF" class="STYLE19">
          <input type="checkbox" name="ids" value="${projectInvestRate.id }" /></td>
          <td height="40" bgcolor="#FFFFFF" ><div align="center">
            ${(page.currentPage-1)*page.pageSize+(s.index+1) }
            </div></td>
                      <td height="40" bgcolor="#FFFFFF" class="STYLE21"><div align="center">${projectInvestRate.rate_name}</div></td>
                      <td height="40" bgcolor="#FFFFFF" class="STYLE21"><div align="center">${projectInvestRate.interval}</div></td>
                      <td height="40" bgcolor="#FFFFFF" class="STYLE21"><div align="center">${projectInvestRate.successRate}</div></td>
                      <td height="40" bgcolor="#FFFFFF" class="STYLE21"><div align="center">${projectInvestRate.content}</div></td>
			<td height="40" bgcolor="#FFFFFF" class="STYLE21">
			<div align="center" class="chao_z_btn">
			<input type="button" value="修改" onclick="modifyDetail('${projectInvestRate.id}')">
			<input type="button"  onclick="deletesingle('${projectInvestRate.id}')"  value="删除"> 
			</div></td>

...
$(document).ready(function(){
	...
	$("#title_content_1").addClass("tab order_tjhover");
	...
	$("#userReg").click(function(){
		...
		$("#submit").click(function() {
			...
			if (attentionName.length==0) {
				alert("收件人姓名不能为空");
				return false;
			};
			var attentionName = $("#attentionAddress").val();
			if (attentionName.length==0) {
				alert("详细地址不能为空");
				return false;
			};
			var attentionName = $("#postCode").val();
			if (attentionName.length==0) {
				alert("邮政编码不能为空");
				return false;
			};
		...
		var J_IptAmount=$("#J_IptAmount").val();
		var attentionName=$("#attentionName").val();
		var attentionAddress=	$("#attentionAddress").val();
		var postCode=$("#postCode").val();
		var mobilePhone=$("#mobilePhone").val();
		var liveCity=$("#liveCity  option:selected").text();
		var liveProvince=$("#liveProvince  option:selected").text();
		var liveArea=$("#liveArea  option:selected").text();
			$.ajax({
				type : "POST",
				async : false,
				cache : false,
				dataType : "json",
				data : {
					priceConvert : goodsprice,
					
					
					goodsName : goodsName,
					num : J_IptAmount,
					attentionName : attentionName,
					attentionAddress : attentionAddress,
					postCode : postCode,
				...
				url : "${ctx}/order/manager/addorder",
				success: function(result){
                   if(result.success){
                	   var id=result.id;
                	   $.ajax({
           				...
           				success: function(data){
           					   ...
           					   var customerInfo=data.data.attentionName+"/"+data.data.attentionAddress+"/"+data.data.mobilePhone;
           					    $("#customerInfo").html(customerInfo);
           					 	$('a[href="orderlist.html"]').attr('href', 'orderlist.html');
           					 	$("#orderId").val(id);
           					 		if(goodsprice=='面议'){
           					 			location.href="${ctx}/order/manager/orderlist.html";
           					 		}else{
									...
									<tbody id="tab">
										...
										<tr>
											...
											<td style="border-left: none;">应付金额：<em id="payment"
												style="color: #f16244; font-size: 32px;">￥${goods.payment}元</em></td>
										</tr>
									</tbody>
								</table>
								<table>
							...
							<div style="display: none;" id="content_2" class="content">
								...
								<p>
									<label>应付金额:</label><em id="paymentResult"
										style="font-size: 28px; color: #f16244;">${goods.payment}</em>
								</p>
								<div class="money_c">
									<table class="money_tab">
										...
										<tr>
											<td><input name="payforMethod"  type="radio"  checked="checked" value="1" /></td>

...
<script type="text/javascript">
    function back(n){
        if(n > 1){
            var page = n-1;
            window.location.href="${ctx}/user/"+page+"/rechargeList";
        }
    }
...
<div class="lyz_tab_left">
    <div class="pro_con111">
        <ul>
            ...
            <li id="one2" onclick="javascript:window.location.href='${ctx}/user/1/rechargeList'" class="hover">充值记录</li>
...
<div style="border:none;padding-top:0" id="con_one_2" class="hover">
    <div class="model-box">
        ...
        <table class="assets-table">
            ...
            <c:forEach var="data" items="${rechargeList}">
                <tr>
                    <td>${data.serialNumber }</td>
                    <td><fmt:formatDate value="${data.dateTime}"  pattern="yyyy-MM-dd HH:mm:ss"/></td>
                    <td>${data.description }</td>
                    <td>+${data.amount } 纳币</td>
                    <td>
                        <c:choose>
                            <c:when test="${data.status == 0 }">交易成功</c:when>
                            <c:when test="${data.status == 1 }">交易失败</c:when>
                            <c:when test="${data.status == 2 }">正在处理</c:when>
                        </c:choose>
                    </td>
                </tr>
            </c:forEach>
        		...
        		Prev
                    ...
                    <c:choose>
                        <c:when test="${page.totalPages < 9 }">
                            <c:forEach var="i" begin="1" end="${page.totalPages }" >
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <a href="${ctx }/user/${i}/rechargeList">${i }</a>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
                        </c:when>
                        <c:when test="${page.currentPage < 8 }">
                            ...
                            <span>...</span>
                            <a href="${ctx }/user/${page.totalPages}/rechargeList">${page.totalPages }</a>
                        </c:when>

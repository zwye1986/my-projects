...
(function($) {
   ...
   jQuery.fn.fancybox = function(settings) {
		opts.settings = $.extend({}, jQuery.fn.fancybox.defaults, settings);
	...
	jQuery.fn.fancybox.start = function(el, o) {
		...
		if (o.overlayShow) {
			...
			$("#fancy_overlay").css({'width': $(window).width(), 'height': $(document).height(), 'opacity':0.0});
			...
			//$("#fancy_overlay").click(jQuery.fn.fancybox.close);
		}
	...
	jQuery.fn.fancybox.changeItem = function(n) {
		...
		if (url.match(/#/)) {
	        ...
	        jQuery.fn.fancybox.showItem('<div id="fancy_div">' + $(target).html() + '</div>');
	...
	jQuery.fn.fancybox.showIframe = function() {
		...
		$("#fancy_frame").show();
		try{
			var documentFrame = document.frames("fancy_frame").document;
			//检测页面是否存在表单【焦点定位在表单第一个可见字段的方法】
			if(documentFrame.forms.length>0){
				for(var i=0;i<documentFrame.forms[0].elements.length;i++){
					var oField=documentFrame.forms[0].elements[i];
	...
	jQuery.fn.fancybox.showItem = function(val) {
		...
		itemLeft	= viewportPos[2] + Math.round((viewportPos[0] - itemSize[0]) / 2) - 20;
		itemTop		= viewportPos[3] + Math.round((viewportPos[1] - itemSize[1]) / 2) - 40;
		if(opts.settings.itemTop != null)
		{
			itemTop = opts.settings.itemTop;
		}
		if(opts.settings.itemLeft != null)
		{
			itemTop = opts.settings.itemLeft;
		}
		var itemOpts = {
			'left':		itemLeft, 
			'top':		itemTop, 
		...
		if (opts.active) {
			...
			if (opts.itemArray[opts.itemNum].o.zoomSpeedIn > 0) {
				...
				$("#fancy_outer").css({
					'top':		itemTop,//opts.itemArray[opts.itemNum].orig.pos.top - 18,
					'left':		itemLeft,//opts.itemArray[opts.itemNum].orig.pos.left - 18,
					'height':	opts.settings.frameHeight-40,//opts.itemArray[opts.itemNum].orig.height,
					'width':	opts.settings.frameWidth-40//itemArray[opts.itemNum].orig.width
				});
	...
	jQuery.fn.fancybox.updateDetails = function() {
		...
		if (opts.itemArray[opts.itemNum].o.hideOnContentClick) {
			$("#fancy_content").click(jQuery.fn.fancybox.close);
		} else {
	...
	jQuery.fn.fancybox.close = function() {
		...
		opts.active	= false;
		if(opts.itemArray == null) return false;
		if (opts.itemArray[opts.itemNum].o.zoomSpeedOut > 0) {
			var itemOpts = {
				'top':		itemTop,//opts.itemArray[opts.itemNum].orig.pos.top - 18,
				'left':		itemLeft,//opts.itemArray[opts.itemNum].orig.pos.left - 18,
//				'height':	opts.settings.frameHeight,//opts.itemArray[opts.itemNum].orig.height,
//				'width':	opts.settings.frameWidth,//itemArray[opts.itemNum].orig.width
				'opacity':	'hide'
			};
		...
		if (jQuery.isFunction(opts.settings.callbackOnClose)) {
			opts.settings.callbackOnClose();
		}
	};
	...
	jQuery.fn.fancybox.showLoading = function() {
		...
		$("#fancy_loading").bind('click', jQuery.fn.fancybox.close);
	...
	jQuery.fn.fancybox.init = function() {
    	...
    	$("#fancy_close").click(jQuery.fn.fancybox.close);
    	
    	try{
    		$("#fancy_outer").draggable({ handle: $("#fancy_inner") });
    	}catch(e){}
	};

(function() {
    ...
    (function() {
        ...
        (function() {
            if (!d)
                if (d = g, "complete" === document.readyState)
                    a.b = g;
                else if (document.addEventListener)
                    document.addEventListener("DOMContentLoaded",
                    f, k), window.addEventListener("load", a, k);
    ...
    var ba = navigator.cookieEnabled, ca = navigator.javaEnabled(), da = navigator.language || navigator.browserLanguage || navigator.systemLanguage || navigator.userLanguage || "", ea = window.screen.width + "x" + window.screen.height, ka = window.screen.colorDepth;
    c.se.push([14, "so.360.cn", "q", 1, ",news"]);
    c.se.push([14, "v.360.cn", "q", 1, "so"]);
    var O = 0, P = Math.round(+new Date / 1E3), Q = "https:" == document.location.protocol ? "https:" : "http:", la = "cc cf ci ck cl cm cp cw ds ep et fl ja ln lo lt nv rnd sb se si st su sw sse v cv lv u api tt".split(" ");
    function ma() {
        if ("undefined" == typeof window["_bdhm_loaded_" + c.id]) { window["_bdhm_loaded_" + c.id] = g;
        var a = this;
        a.a = {};
        a.q = [];
        a.p = {push: function() {
...
ma.prototype = {getData: function(a) {
        try {
        ...
        try {
            A(a, b, {domain: oa(),path: pa(),f: d}), d ? G(a, b, d) : s(a, b)
        } catch (e) {
        }
    },k: function(a) {
        if (function(a) {
            ...
            for (var b = a.length - 1; 0 <= b; b--) {
                var d = a[b];
                if (("[object Number]" !== {}.toString.call(d) || !isFinite(d)) && "[object String]" !== {}.toString.call(d) && d !== g && d !== k)
                    return k
            }
            return g
        }(a)) {
            ...
            switch (a[0]) {
                ...
                case "_trackEvent":
                    ...
                    b(a[2]) + (a[3] ? "*" + b(a[3]) : "") + (a[4] ? "*" + b(a[4]) : ""), R(this));
                    break;
                case "_setCustomVar":
                    if (4 > a.length)
                        break;
                    ...
                    if (0 < d && 6 > d && 0 < e && 4 > e) {
                        this.c++;
                        for (var f = (this.a.cv || "*").split("!"), n = f.length; n < d - 1; n++)
                            f.push("*");
                        f[d - 1] = e + "*" + b(a[2]) + "*" + b(a[3]);
                        this.a.cv = f.join("!");
...
function sa(a, b) {
    var d = z("Hm_unsent_" + c.id) || "", e = a.a.u ? "" : "&u=" + encodeURIComponent(document.location.href), d = encodeURIComponent(b.replace(/^https?:\/\//, "") + e) + (d ? "," + d : "");
    s("Hm_unsent_" + c.id, d)
}
function R(a) {
    a.a.rnd = Math.round(2147483647 * Math.random());
    a.a.api = a.a.api || a.c ? a.a.api + "_" + a.c : "";
    var b = Q + "//hm.baidu.com/hm.gif?" + ta(a);
    a.a.api = 0;
    a.c = 0;
    sa(a, b);
    C(b, function(a) {
...
function na(a) {
    try {
        ...
        a: if (document.referrer) {
            for (var p = 0, wa = c.se.length; p < wa; p++)
                if (RegExp("(^|\\.)" + c.se[p][1].replace(/\./g, "\\.")).test(r(document.referrer))) {
                    ...
                    if (x || !(2 != c.se[p][0] && 14 != c.se[p][0] && 17 != c.se[p][0])) {
                        1 == c.se[p][0] && -1 < document.referrer.indexOf("cpro.baidu.com") && (x = "");
                        a.a.se = c.se[p][0];
                        for (var xa = a.a, m = c.se[p], H = 0, ya = 2 == m[3] ? m[1] + "\\/" : "", za = 1 == m[3] ? "\\." +
                        m[1] : "", T = m[4].split(","), m = 0, Aa = T.length; m < Aa; m++)
            ...
            d = P - O > c.vdur ? 1 : 4;
        b = 4 != d ? 1 : 0;
        if (y = a.getData("Hm_lvt_" + c.id)) {
            l = y.split(",");
            for (var E = l.length - 1; 0 <= E; E--)
                13 == l[E].length && (l[E] = "" + Math.round(l[E] / 1E3));
            for (; 2592E3 < P - l[0]; )
                l.shift();
            n = 4 > l.length ? 2 : 3;
            for (1 ===
            b && l.push(P); 4 < l.length; )
                l.shift();
            y = l.join(",");
            f = l[l.length - 1]
        } else
            y = P, f = "", n = 1;
        a.setData("Hm_lvt_" + c.id, y, c.age);
        a.setData("Hm_lpvt_" + c.id, P);
        e = P == a.getData("Hm_lpvt_" + c.id) ? "1" : "0";
        if (0 == c.nv && U(document.location.href) && ("" == document.referrer || U(document.referrer)))
            b = 0, d = 4;
        a.a.nv = b;
        a.a.st = d;
        a.a.cc = e;
        ...
        0 == a.a.nv ? ra() : S(".*");
        if (false) {
            var u, v = c.icon.split("|"), W = "http://tongji.baidu.com/hm-web/welcome/ico?s=" + c.id, X = ("http:" == Q ? "http://eiv" : "https://bs") + ".baidu.com" + v[0] + "." + v[1];
            switch (v[1]) {
                case "swf":
                    var ga = v[2], ha = v[3], K = "s=" + W, t = "HolmesIcon" +
                    P;
                    u = '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" id="' + t + '" width="' + ga + '" height="' + ha + '"><param name="movie" value="' + X + '" /><param name="flashvars" value="' + (K || "") + '" /><param name="allowscriptaccess" value="always" /><embed type="application/x-shockwave-flash" name="' + t + '" width="' + ga + '" height="' + ha + '" src="' + X + '" flashvars="' + (K || "") + '" allowscriptaccess="always" /></object>';
                    break;
                case "gif":
                    u = '<a href="' + W + '" target="_blank"><img border="0" src="' + X + '" width="' + v[2] + '" height="' +
                    v[3] + '"></a>';
                    break;
                default:
                    u = '<a href="' + W + '" target="_blank">' + v[0] + "</a>"
            }
            document.write(u)
        }
        var L = document.location.hash.substring(1), Ba = RegExp(c.id), Ca = -1 < document.referrer.indexOf("baidu.com") ? g : k;
        if (L && Ba.test(L) && Ca) {
                ...
                switch (B[0]) {
                    ...
                    case "_setAutoPageview":
                        ...
                        B.length) {
                            var Y = B[1];
                            if (k === Y || g === Y)
                                a.a.api |= 2, window._bdhm_autoPageview = Y
                        }
                }
            }
        if ("undefined" === typeof window._bdhm_account || window._bdhm_account === c.id) {
            ...
            var N = window._hmt;
            if (N && N.length)
                for (var w = 0, Ea = N.length; w < Ea; w++)
                    a.k(N[w]);
            window._hmt = a.p
        }
        if ("undefined" === typeof window._bdhm_autoPageview || window._bdhm_autoPageview === g)
            a.h = g, a.a.et = 0, a.a.ep = "", R(a)
    } catch (ja) {
        a = [], a.push("si=" + c.id), a.push("n=" + encodeURIComponent(ja.name)), a.push("m=" + encodeURIComponent(ja.message)),
        a.push("r=" + encodeURIComponent(document.referrer)), C(Q + "//hm.baidu.com/hm.gif?" + a.join("&"))
    }
}
function ta(a) {
...
function qa() {
    var a = "Hm_cv_" + c.id;try {
        if (A(a, "", {domain: oa(),path: pa(),f: -1}), window.sessionStorage && window.sessionStorage.removeItem(a), window.localStorage)
...
function pa() {
    for (var a = 0, b = c.dm.length; a < b; a++) {
        var d = c.dm[a];
        if (-1 < d.indexOf("/") && Da(document.location.href, d))
            return d.replace(/^[^\/]+(\/.*)/, "$1") + "/"
    }
    return "/"
}

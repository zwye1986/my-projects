...
Vanadium.addValidatorTypes([
  ...
  if (typeof(VanadiumCustomValidationTypes) !== "undefined" && VanadiumCustomValidationTypes) Vanadium.addValidatorTypes(VanadiumCustomValidationTypes);
	...
	$(document).ready(function(){
		...
		$("#resultList tbody tr").click(function(){
		...
		$("#delOrder").click(function(){
			var orderId = $(".choose input[name='orderId']").val();
			if($.type(orderId) === 'undefined' || $.type(orderId) === 'null'){
				alert("请选择一条订单纪录进行删除!");
				return;
			}
			if(confirm("您确定要删除选中订单纪录吗？")){
				$.ajax({
					   type: "POST",
					   async: false,
					   cache: false,
					   ...
					   success: function(data){
						   if(data.resCode == 2){
							   window.location.href="${ctx}/order.html";
						   }else{
							   alert(data.resMsg);
							   return;
						   }
		...
		$("#modifyOrder").click(function(){
			...
			window.location.href = "${ctx}/modifyOrder?orderId="+orderId;
		});
	$("#adduser").click(function (){
		$("div.add_c").css("display","block");
	});

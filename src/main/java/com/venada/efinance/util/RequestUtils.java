 ...
 * 
 * @author    yma
 * @version   V1.00 2011-10-05[版本号, YYYY-MM-DD]
 * @see       [相关类/方法]
...
public class RequestUtils {
	 ...
	 *            current HTTP request
	 * @param name
	 *            the name of the parameter @ a subclass of ServletException, so
	 *            it doesn't need to be caught
	 */
	 ...
	 *            current HTTP request
	 * @param name
	 *            the name of the parameter with multiple possible values @ a
	 *            subclass of ServletException, so it doesn't need to be caught
	 */
	public static int[] getRequiredIntParameters(ServletRequest request, String name) {
		...
		if (values.length == 1) {
			return INT_PARSER.parseInts(name, StringUtils.split(values[0], ","));
		}
	 ...
	 * @param name
	 *            the name of the parameter
	 * @return the Long value, or <code>null</code> if not present @ a subclass
	 *         of ServletException, so it doesn't need to be caught
	 */
	...
	public static long[] getRequiredLongParameters(ServletRequest request, String name) {
		...
		if (values.length == 1) {
			return LONG_PARSER.parseLongs(name, StringUtils.split(values[0], ","));
		}
	 ...
	 * @param name
	 *            the name of the parameter
	 * @return the Float value, or <code>null</code> if not present @ a subclass
	 *         of ServletException, so it doesn't need to be caught
	 */
	...
	public static float[] getRequiredFloatParameters(ServletRequest request, String name) {
		...
		if (values.length == 1) {
			return FLOAT_PARSER.parseFloats(name, StringUtils.split(values[0], ","));
		}
	...
	public static double[] getRequiredDoubleParameters(ServletRequest request, String name) {
		...
		if (values.length == 1) {
			return DOUBLE_PARSER.parseDoubles(name, StringUtils.split(values[0], ","));
		}
	...
	public static boolean[] getRequiredBooleanParameters(ServletRequest request, String name) {
		...
		if (values.length == 1) {
			return BOOLEAN_PARSER.parseBooleans(name, StringUtils.split(values[0], ","));
		}
	...
	public static String getStringParameter(ServletRequest request, String name) {
		...
		String value = getRequiredStringParameter(request, name);
	...
	private abstract static class ParameterParser {
		...
		protected final Object parse(String name, String parameter) {
			...
			try {
				...
				throw new IllegalArgumentException("Required " + getType() + " parameter '" + name
